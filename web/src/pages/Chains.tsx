  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1 bg-black text-white px-3 py-1.5 rounded text-sm"><Plus size={14} /> 添加链</button>
      </div>
      {isLoading ? <p className="text-gray-500">加载中...</p> : (
        <table className="w-full text-sm border-collapse">
          <thead><tr className="border-b text-left text-gray-500">
            ...
            <th className="py-2 px-2">确认数</th><th className="py-2 px-2">轮询</th><th className="py-2 px-2">状态</th><th className="py-2 px-2"></th>
          </tr></thead>
          <tbody>{chains.map((c) => (
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <form onSubmit={submit} className="bg-white rounded-lg p-6 w-96 space-y-3">
        <h3 className="text-lg font-bold">添加链</h3>
        <input name="id" placeholder="链 ID（如 eth-mainnet）" required className="w-full border rounded px-3 py-1.5 text-sm" />
        ...
        <input name="rpc_http" placeholder="RPC HTTP 地址" required className="w-full border rounded px-3 py-1.5 text-sm" />
        <input name="rpc_ws" placeholder="RPC WS 地址（可选）" className="w-full border rounded px-3 py-1.5 text-sm" />
        ...
        <input name="poll_interval_ms" type="number" defaultValue={3000} placeholder="轮询间隔 (ms)" className="w-full border rounded px-3 py-1.5 text-sm" />
